...
for t=2:TT
  % f(:,t)= 0.5*f(:,t-1)+sqrt(1-0.5^2)*(repmat(mu,1,1) + randn(1,2)*R1)'; % k by TT
   f(:,t)= 0.5*f(:,t-1)+sqrt(1-0.5^2)*(mvnrnd(mu,sigmau))'; % k by TT
end 
 f2=f;
load=(-0.5+rand(m+1,k,N));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
%tau=1;
%indi_u=sqrt(tau)*((chi2rnd(1)-1)/sqrt(2))*mvnrnd(mu,sigma_idi,N); % gen individual effect (N by m+1)
b0=1;
b1=1;
...
for tt=2:TT
 ...
 % y(:,:,tt)=Phi*y(:,:,tt-1)+indi_u'+(delta(tt))*ones(m+1,N)+(fac)+(idi_error(:,:,tt))'; 
...
options_ML = optimoptions(@fmincon, 'Display','off','GradObj','off','Hessian','off','Algorithm','interior-point' ,'TolX',10^(-5),'TolFun',10^(-5),'TolCon',10^(-6), 'MaxIter',400);
  ...
  lb=[ -1*ones(2*(m+1),1)      ;[0.0001; 0.0001;-inf]       ;[0.0001; 0.0001; -inf]     ; -inf*ones((T0)*(m+1),1)];
 ub=[ 1*ones(2*(m+1),1)    ; inf*ones(3,1)          ; inf*ones(3,1)   ; inf*ones((T0)*(m+1),1) ];
...
filename='case1_2factor_3con.mat';
save(filename);
toc
