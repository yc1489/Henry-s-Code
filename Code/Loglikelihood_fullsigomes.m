...
sqrSigmaE = inv(SigmaE)^(0.5); %try inv
...
%logL=-((T1)/2)*(real(log(det(Sigmae))))-(1/2)*(real(log(det(BigOmega))))-(1/2)*sum(log(lambda(1:k)))+(1/2)*(sum(lambda(1:k))-k)-(1/2)*sum(lambda(1:(T1)));

logL=-(N/2)*(log(abs(det(SigmaE))))-(N/2)*sum(log(lambda(1:k)))+(N/2)*(sum(lambda(1:k))-k)-(N/2)*sum(lambda(1:(T0)));
