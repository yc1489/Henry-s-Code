...
Dy=y(:,:,2:T0)-y( :, :,1:T0-1);  %gen data m by N by T-1  (Dy1,...,DyT)


DW1_N = zeros(T1*(m+1),T1*(m+1),N); % T x dim_phi x N   for d
    for i=1:N
       DW1_N(:,:,i) =  eye(T1*(m+1)); 
    end
...
DW2_N = zeros(T1*(m+1),N); % T x dim_phi x N   for y_t-1
    for ii=1:N
       DW2_N(:,ii) =  [zeros(m+1,1);Dy(1:(T1-1)*(m+1),ii)]; 
    end    
...
options_ML = optimoptions(@fmincon, 'Display','off','GradObj','off','Hessian','off','Algorithm','interior-point' ,'TolX',10^(-5),'TolFun',10^(-5),'TolCon',10^(-6), 'MaxIter',400);
  ...
  lb=[ -inf*ones(2*(m+1),1)      ;[0.0001; 0.0001;-inf]       ;[0.0001; 0.0001; -inf]     ; -inf*ones((T1)*(m+1),1)];
 ub=[ inf*ones(2*(m+1),1)    ; inf*ones(3,1)          ; inf*ones(3,1)   ; inf*ones((T1)*(m+1),1) ];
...
% sigma_ini3=[sigma_ini1,sigma_ini2];
 ...
 tdum_ini1=rand(1,(T1-1)*(m+1))*2-ones(1,(T1-1)*(m+1));
 
theta_ini=[ gam_ini1,  omega_ini3, sigma_ini3,d_ini,tdum_ini1]';


[theta_ML_j,fval_j,exitflag_j,~,~,~,~] = fmincon(@(theta)Loglikelihood_fullsigomes(N,T1,k,m,Dy,DW1_N,DW2_N,theta,theta_idx),theta_ini,[],[],[],[],lb,ub,'constraints_fullsigome',options_ML);
